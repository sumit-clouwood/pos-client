...
jQuery.fn.SharpCalendar = jQuery.fn.SC = function(options) {
  ...
  if (typeof options.showDayNames === 'undefined') {
    options.showDayNames = true
  }
  if (typeof options.showDayNamesOnTop === 'undefined') {
    options.showDayNamesOnTop = false
  }
  if (typeof options.doubleDigitsDays === 'undefined') {
    options.doubleDigitsDays = false
  }
  ...
  if (typeof options.monthNames === 'undefined') {
    options.monthNames = options.vertical
      ? [
          'January',
          'February',
          'March',
  ...
  if (typeof options.dayNames === 'undefined') {
    options.dayNames = options.vertical
      ? [
          'Sunday',
          'Monday',
          'Tuesday',
  ...
  if (typeof options.combineMonthYear === 'undefined') {
    options.combineMonthYear = false
  }
  if (typeof options.allowSelectSpans === 'undefined') {
    options.allowSelectSpans = false
  }
  if (typeof options.selectedDatesObj === 'undefined') {
  ...
  if (typeof options.sizes === 'undefined') {
    options.sizes = 'auto'
  } // options: auto, equal, none
  // --------------------------  End default option values --------------------------
  var calendar = { currentDate: options.date },
    sharpCContainer = this.eq(0)
      .addClass('SCBox')
      .addClass('SCTbl'),
    ...
    var fillYears = function(date) {
      ...
      fillMonths = function(date) {
        ...
        for (i = -options.months; i <= options.months; i += 1) {
          ...
          div = $('<div>')
            ...
            .html(
              '<div><span>' +
                options.monthNames[d.getMonth()] +
                (options.combineMonthYear
                  ? "'" +
                    d
                      .getFullYear()
                      .toString()
          ...
          if (
            d.getFullYear() === calendar.currentDate.getFullYear() &&
            d.getMonth() === calendar.currentDate.getMonth()
          ) {
            div.addClass('SCSel')
          }
      ...
      fillDays = function(date) {
        ...
        if (options.showDayArrows) {
          ...
          div.wrap('<div class="prev"/>').wrap('<div/>')
        ...
        for (i = -options.days; i <= options.days; i += 1) {
          ...
          div.html(
            '<div><div class="SCDayNum">' +
              (options.doubleDigitsDays
                ? ('0' + d.getDate()).slice(-2)
                : d.getDate()) +
              '</div></div>'
          )
          ...
          if (
            d.getFullYear() === calendar.currentDate.getFullYear() &&
            d.getMonth() === calendar.currentDate.getMonth() &&
            d.getDate() === calendar.currentDate.getDate()
          ) {
            div.addClass('SCSel')
          }
          ...
          div.wrap('<div class="SCTbl wd_' + d.getDay() + '"/>').wrap('<div/>')
        ...
        if (options.showDayArrows) {
          ...
          div = $('<div>')
            ...
            .attr('millis', d.getTime())
          div.html('<div class="SCDayNum"></div>')
          div.wrap('<div class="next"/>').wrap('<div/>')
          Days.append(div.parent().parent())
        }
      },
      ...
      finish = function() {
        var auxH =
            sharpCContainer.height() === 0
              ? sharpCContainer.parent().height()
              : sharpCContainer.height(),
          auxW =
            sharpCContainer.width() === 0
              ? sharpCContainer.parent().width()
              : sharpCContainer.width(),
          marginH =
            parseInt(
              Months.find('.SCTbl')
                .css('margin-left')
        ...
        if (options.sizes === 'auto' || options.sizes === 'equal') {
          ...
          setSizes(
            ...
            marginH,
            marginV,
            options.prvtMonthNumCells
          )
          setSizes(Days, auxH, auxW, marginH, marginV, options.prvtDayNumCells)

          if (!options.vertical) {
            Years.find('.SCTbl .SCElement>div').height(
              Years.find('.SCSel')
                .parent()
                .parent()
                .outerHeight()
            )
            Months.find('.SCTbl .SCElement>div').height(
              Months.find('.SCSel')
                .parent()
                .parent()
                .height()
            )
            Days.find('.SCTbl .SCElement>div').height(
              Days.find('.SCSel')
                .parent()
                .parent()
                .height()
            ...
            if (options.showDayArrows) {
              Days.find('.prev .SCElement').height(
                Days.find('.SCTbl .SCElement').height() + 'px'
              )
              Days.find('.prev .SCElement .SCDayNum').css(
                'line-height',
                Days.find('.SCTbl .SCElement').height() + 'px'
              )
              Days.find('.next .SCElement').height(
                Days.find('.SCTbl .SCElement').height() + 'px'
              )
              Days.find('.next .SCElement .SCDayNum').css(
                'line-height',
                Days.find('.SCTbl .SCElement').height() + 'px'
              )
            }
  ...
  function animate(old_d, new_d, time, increment) {
    ...
    if (
      old_d.getDate() !== new_d.getDate() ||
      old_d.getMonth() !== new_d.getMonth() ||
      old_d.getFullYear() !== new_d.getFullYear()
    ) {
