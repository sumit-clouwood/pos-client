...
<div class="dm-order-screens dm-order-screen-change" id="home-delivery-order">
    ...
    <div class="dm-ready-order-wrapper" id="dm-waiting-for-pick">
        <div class="select-driver"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">Select Driver</button>
            <ul class="dropdown-menu">
                <li v-for="(driver, index) in  driverList" :key="index">
                    <img :src="driver.driverImagePath" class="pull-left driverImg" @error="imageLoadError()">
...
export default {
  ...
  data() {
    return {
      readyDetails: {'moreDetails': true, 'action': 'Ready', 'nextOrderStatus': 'ready'},
      waitingOrder: {'moreDetails': false, 'action': '', 'driverId': '', 'nextOrderStatus': 'in-progress'},
      deliveredDetails: {'moreDetails': true, 'action': 'Ready', 'nextOrderStatus': 'delivered'},
    }
  },
  components: {
    DMHomeDeliverySubMenu,
