<template>
    <div class="container-fluid row running-orders-dinein" id="running-orders-show" style="">
            ...
            <div class="running-order-table-wrap">
                <table class="table" id="running-order">
                    ...
                    <tr class="dine-table-heading">
                        ...
                        <th width="400px">{{ _t('ORDER NUMBER') }}</th>
                        <th width="200px">{{ _t('AMOUNT') }}</th>
                        <th width="200px">{{ _t('STATUS') }}</th>
                        <th width="340px">{{ _t('DINING FOR') }}</th>
                    </tr>
                    </thead>
                    <tbody v-if="dineInOrderDetails">
                    <tr :key="index" class="dine-table-content" v-for="(order, index) in dineInOrderDetails">
                        <td class="dine-order-tabel"><span>{{ getTableNumber(order._id) }}</span></td>
                        ...
                        <td class="dine-order-amt"> {{ order.balance_due + ' ' + order.currency }}</td>
                        <td :class="getOrderStatus(order.order_status)"><span>{{ order.order_status }}</span></td>
                        <!--<td><span>{{ order.order_status }}</span></td>-->
                        <td class="order-time-det"><span>5 Minutes</span>
                            <div class="dining-for-button"><span class="dinefor-reprint"><svg
                                    height="19" viewBox="0 0 22 19" width="22" xmlns="http://www.w3.org/2000/svg">
        <g fill="#62BB31" fill-rule="nonzero">
            <path d="M2.68 7.096c0-2.825-.274-2.568 1.51-2.568V1.113c0-.53.438-.963.968-.963h11.684c.53 0 .967.433.967.963v3.415c1.785 0 1.511-.257 1.511 2.568.832 0 2.44-.26 2.44.95v5.993a.96.96 0 0 1-.963.946h-2.988v2.832a.971.971 0 0 1-.967.963H5.158a.971.971 0 0 1-.967-.963v-2.832H1.203a.96.96 0 0 1-.964-.946V8.046c0-.527.44-.95.964-.95H2.68zM3.702 5.55v1.546h.489V5.55h-.489zm14.596 0h-.489v1.546h.489V5.55zm-.489 7.425v.987h2.93V8.12H1.261v5.843H4.19c0-.499-.177-1.951.967-1.951h11.684c.593 0 .964.596.964.964h.003zm-1.022 4.78l-.007-4.718H5.217l-.004 4.717h11.574zM5.213 1.171v5.924h11.574V1.172H5.213z"></path>
            <path d="M17.518 9.8c-.672 0-.672-1.023 0-1.023h1.643c.676 0 .676 1.023 0 1.023h-1.643z"></path>
        </g>
    </svg>Re-Print</span><span class="dinefor-paynow"><svg height="21" viewBox="0 0 30 21" width="30"
        ...
        <g fill="#FFF" fill-rule="nonzero">
            ...
            <path d="M12.844 9.08a3.64 3.64 0 1 0 7.28-.001 3.64 3.64 0 0 0-7.28 0zm6.077 0c0 .675-.272 1.28-.714 1.724a2.428 2.428 0 0 1-1.724.714c-.675 0-1.28-.272-1.724-.714a2.428 2.428 0 0 1-.714-1.724c0-.675.272-1.281.714-1.724a2.428 2.428 0 0 1 1.724-.714c.675 0 1.28.272 1.724.714.442.443.714 1.049.714 1.724z"></path>
            <circle cx="10.222" cy="5.096" r="1.001"></circle>
            <circle cx="22.782" cy="13.001" r="1.001"></circle>
            <path d="M25.21 19.19a1.2 1.2 0 0 1-.851.355H2.675a1.2 1.2 0 0 1-.851-.355 1.2 1.2 0 0 1-.355-.851V5.69c0-.331.134-.63.355-.851a1.2 1.2 0 0 1 .851-.355V3.283A2.413 2.413 0 0 0 .267 5.69v12.65a2.413 2.413 0 0 0 2.408 2.407H24.36a2.413 2.413 0 0 0 2.407-2.408h-1.201c0 .33-.135.63-.355.851z"></path>
        </g>
    </svg>Pay Now</span></div>
                        </td>
                    </tr>
                    </tbody>
                </table>
    ...
    methods: {
      /*getOrderStatus(order_status) {
        if (order_status === CONST.ORDER_STATUS_ON_HOLD || order_status === CONST.ORDER_STATUS_IN_PROGRESS) {
          this.orderStatusClass = 'running-order-details'
        } else {
          this.orderStatusClass = 'done-soon-order'
        }
      }*/
