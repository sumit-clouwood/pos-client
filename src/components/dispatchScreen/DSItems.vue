<template>
    <div class="inner-content-dm" v-if="dispatchOrders">
        <div class="block1-wrap-dp" v-for="(order, index) in dispatchOrders" :key="index" :id="'ds_'+order.id">
            <div class="dp-box-wrap">
                <div class="dp-inner-box-wrap">
                    <div class="left-inner-dp">
                        <h5 class="text-capitalize">{{ order.order_type }} <span> {{order.items.length}}</span></h5>
                    ...
                    <div class="right-inner-dp">
                        <button class="rectangle-sample" @click="collected(order.id)">Collected ?</button>
                        <h1 id="dp-order-no">{{index + 1}}</h1>
                    </div>
                </div>
                <div class="dp-box-footer">
                    {{humanDateTime({order_created: order.created_at, order_no: order.order_no})}}
