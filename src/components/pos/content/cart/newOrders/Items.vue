  ...
  <div class="main-orders-list" v-if="items">
    ...
    >
      <div class="main-orders-list-item-title color-text">
        ...
        <div class="orders-amount">{{ formatPrice(itemGrossPriceDiscounted(item)) }}</div>
        <div class="orders-close" @click.prevent="removeCurrentOrder({ item: item, index: index })">
          <svg
      ...
      <div class="main-orders-list-item-buttons">
        ...
        <div class="button-plus" data-toggle="modal" data-target="#POSOrderItemOptions">
          <div class="button-plus-icon" @click="setActiveItem({ orderItem: item, index: index })">
            <svg class="color-text" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">
              <path
