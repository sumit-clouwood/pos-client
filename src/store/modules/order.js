...
const actions = {
  addToOrder({ state, commit, rootState, dispatch }, item) {
    ...
    if (
      ...
      item.price = isNaN(parseFloat(item.get_item_location_price.menu_location_price)) ? 0 : parseFloat(item.get_item_location_price.menu_location_price)
    } else {
      item.price = isNaN(parseFloat(item.item_price)) ? 0 : parseFloat(item.get_item_location_price.menu_location_price)
    }
